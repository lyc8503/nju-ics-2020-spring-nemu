...
#define __sign(x) (f2u(x) & 0x80000000)
#define __scale(x) (__sign(x) ? -(x) : (x))
float pow(float x, float y)
{
   /* we only compute x^0.333 */
   float t, t2, dt, next_t = 2.;
...
--*/
{
   ...
   {
      sDamage = ((wAttackStrength << 1) - (1.6 * wDefense) + 0.5);
   }
   else if (wAttackStrength > (0.6 * wDefense))
   {
      sDamage = (wAttackStrength - (0.6 * wDefense) + 0.5);
   }
   else
   {
      sDamage = 0;
...
--*/
{
   ...
   // Formula courtesy of palxex and shenyanduxing
   //
   wMagicStrength = ((RandomFloat(10, 11) * wMagicStrength));
   wMagicStrength /= 10;
...
--*/
{
   g_Battle.flTimeChargingUnit = pow((PAL_GetPlayerDexterity(0) + 5), 0);
   g_Battle.flTimeChargingUnit = (g_Battle.flTimeChargingUnit / PAL_GetPlayerDexterity(0));

   if (gpGlobals->bBattleSpeed > 1)
   {
      g_Battle.flTimeChargingUnit = (g_Battle.flTimeChargingUnit / ((gpGlobals->bBattleSpeed + 1) >> 1));
   }
   else
   {
      g_Battle.flTimeChargingUnit /= 1.2f;
      g_Battle.flTimeChargingUnit = (g_Battle.flTimeChargingUnit / 1.2);
   }
}
...
--*/
{
   ...
   for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
   {
      ...
      case kFighterWait:
         ...
         g_Battle.rgPlayer[i].flTimeMeter += (PAL_GetTimeChargingSpeed(wDexterity) * g_Battle.rgPlayer[i].flTimeSpeedModifier);
         break;
      ...
      if (g_Battle.UI.state == kBattleUIWait)
      {
         ...
         if (i > gpGlobals->wMaxPartyMemberIndex)
         {
            ...
            for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
            {
               ...
               g_Battle.ActionQueue[j].wDexterity = (int)((RandomFloat(0.9, 1.1) * g_Battle.ActionQueue[j].wDexterity));
            ...
            for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
            {
               ...
               else
               {
                  ...
                  wDexterity = (int)((RandomFloat(0.9, 1.1) * wDexterity));
   ...
   case kBattleActionAttack:
      if (sTarget != -1)
      {
         ...
         for (t = 0; t < (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusDualAttack] ? 2 : 1); t++)
         {
            ...
            sDamage = (int)((RandomFloat(1, 1.125) * sDamage));
