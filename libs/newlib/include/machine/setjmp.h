...
#ifdef _JBTYPE
typedef _JBTYPE sigjmp_buf[_JBLEN + 1 + ((sizeof(_JBTYPE) + sizeof(sigset_t) - 1) / sizeof(_JBTYPE))];
#else
...
#define siglongjmp(env, val)                                                                      \
  __extension__({                                                                                 \
    sigjmp_buf *_sjbuf = &(env);                                                                  \
    ((((*_sjbuf)[_SAVEMASK]) ? __SIGMASK_FUNC(SIG_SETMASK, (sigset_t *)((*_sjbuf) + _SIGMASK), 0) \
                             : 0),                                                                \
     longjmp(*_sjbuf, val));                                                                      \
  })
...
#define siglongjmp(env, val) ((((env)[_SAVEMASK]) ? __SIGMASK_FUNC(SIG_SETMASK, (sigset_t *)((env) + _SIGMASK), 0) : 0), \
                              longjmp(env, val))
